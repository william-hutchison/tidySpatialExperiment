...
[![R build
status](https://github.com/stemangiola/tidySpatialExperiment/workflows/R-CMD-check-bioc/badge.svg)](https://github.com/stemangiola/tidySpatialExperiment/actions)
<!-- badges: end -->

**Brings SpatialExperiment to the tidyverse!**

Website:
[tidySpatialExperiment](https://stemangiola.github.io/tidySpatialExperiment/articles/introduction.html)
...
tidySpatialExperiment provides a bridge between Bioconductor
...
viewing the Bioconductor *SpatialExperiment* object as a tidyverse
tibble, and provides SpatialExperiment-compatible *dplyr*, *tidyr*,
...
This is a *SpatialExperiment* object but it is evaluated as a tibble.
So it is compatible both with SpatialExperiment and tidyverse.
...
```
    ...
    ## # A SpatialExperiment-tibble abstraction: 80 × 17
...
```
    ...
    ## # A SpatialExperiment-tibble abstraction: 80 × 18
    ## # [90mFeatures=230 | Cells=80 | Assays=counts, logcounts[0m
...
```
    ...
    ## tidySpatialExperiment says: This operation lead to duplicated cell names. A data frame is returned for independent data analysis.

![](man/figures/unnamed-chunk-10-1.png)<!-- -->

# Preprocess the dataset

We can also treat `pbmc_small_polished` as a *SpatialExperiment*
...
tidySpatialExperiment collection, we can use `as_tibble` to
permanently convert `tidySpatialExperiment` into a tibble.
...
```
    ...
    ## # A SpatialExperiment-tibble abstraction: 80 × 19
    ## # [90mFeatures=230 | Cells=80 | Assays=counts, logcounts[0m
...
SpatialExperiment, tidyverse functions and tidyHeatmap
...
SpatialExperiment framework and *scater*.
...
```
    ...
    ## tidySpatialExperiment says: from version 1.3.1, the special columns including
...
```
    ...
    ## = "all") %>% : tidySpatialExperiment says: from version 1.3.1, the special
    ...
    ## # A SpatialExperiment-tibble abstraction: 80 × 23
    ## # [90mFeatures=230 | Cells=80 | Assays=counts, logcounts[0m
...
A powerful tool we can use with tidySpatialExperiment is tidyverse
...
tidyverse and SpatialExperiment seamlessly.
